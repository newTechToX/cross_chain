...
def _post (url, payload, extraHeaders):
    s = requests.Session()
    s.mount('https://', HTTPAdapter(max_retries=Retry(total=10, method_whitelist=frozenset(['GET', 'POST'])))) # 设置 post()方法进行重访问
    while true:
        try:
            resp = s.post(url, json=payload, headers=extraHeaders, timeout=8)
            s.keep_alive = False
            return resp
...
def query_web3(baseUrl, fromBlock, toBlock, topics, addresses=[], extraHeaders={}):
    ...
    resp = _post(baseUrl, payload, extraHeaders)
...
def query_web3_latest_block(chain, extraHeaders={}):
    ...
    resp = _post(url, payload, extraHeaders)
    resp.raise_for_status()
    resp = resp.json()
